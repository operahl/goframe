...
func CheckFiterString(str string) bool {
	pregStr := regexp.MustCompile(`[$|¥|￥|·|'|‘|、|\\，|,|。|.|～|~|！|!|＠|@|＃|#|％|%|＆|&×|（|）|(|)|｛|｝|{|}|\[|\]|【|】|/：|:|\*|＊|\+|＋|-|－|—|=|＝|<|﹤|︳|……|^|-|∕|¦|‖|︴|“|《|》|？|?|…|…|｜|：|“|《|》|=|"|;|,|.|*]`)
	isTrue := pregStr.FindAllString(str, -1)
	if len(isTrue) > 0 {
		return false
	}
...
func Int64ToInt32(from int64) int {
	intstring := Int64ToString(from)
	to, _ := strconv.Atoi(intstring)
	return to
}
